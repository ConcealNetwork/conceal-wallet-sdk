...
**Current JS status:** float32 op-for-op mirror (`Math.fround` + `Math.trunc`) — matches daemon.
...
if (termQuarters === 0) return 0; // C++ pow(..., 0) → m8=0 → interest 0
if (termQuarters > 20) throw new Error(`V2i: termQuarters ${termQuarters} out of range 1..20`);
const entry = INVESTMENT_M8.find(e => e.quarter === termQuarters);
const m8 = entry.m8;  // already Math.fround'd, bit-identical to C++
```

`termQuarters === 0` returns 0 (C++ result). `termQuarters > 20` throws `Error` (fail closed — table covers 1…20 only; `DEPOSIT_MAX_TERM_V1 = 64800 × 20`; consensus rejects longer terms at accept time). C++ `calculateInterest` itself does not clamp.
...
The daemon stores interest as a uint64 computed from float32 arithmetic. Wallets that
persisted interest from the **pre-parity** float64 JS path may be off by ±1–8 atomic
units. On **withdrawal**, the daemon recomputes interest server-side and **fail-closes**
the transaction if the presented interest does not match. Clients should treat locally
computed interest as an estimate only until resync; the canonical value is always the daemon's.
...
- **`tests/interest-parity.test.ts`** — parity test suite. V1 / V2i / V2w / V3 all GREEN (float32 rewrite).
